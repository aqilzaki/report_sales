...
# ======================== RESELLER HIERARCHY WITH PROFIT ========================
def get_reseller_hierarchy_with_profit(page=1, limit=10):
    """Ambil reseller root dengan pagination, cek downline, dan hitung profit"""
    try:
        ...
        for root_row in root_results:
            ...
            downline_results = db.session.execute(
                downline_query, {"upline_kode": root_kode}
            ).fetchall()
            ...
            for downline_row in downline_results:
                ...
                profit_result = db.session.execute(
                    profit_query, {"reseller_kode": downline_kode}
                ).fetchone()
        ...
        # Hitung total root reseller (untuk info total pages)
        total_query = text("""
            SELECT COUNT(*) 
            FROM reseller
            WHERE kode_upline IS NULL OR kode_upline = '' OR kode_upline = '0'
        """)
        total_roots = db.session.execute(total_query).scalar()

        return {"page": page, "limit": limit, "total": total_roots, "data": hasil}
